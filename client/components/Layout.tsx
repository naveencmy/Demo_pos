...
export const Layout = ({ children }: LayoutProps) => {
  ...
  const visibleNavItems = navItems.filter((item) => {
    if (!currentUser) return item.path === "/";

    if (userRole === "owner") return true;
    if (userRole === "manager") {
      return ["/", "/sales", "/purchase", "/inventory", "/parties", "/reports"].includes(
        item.path
      );
    }
    if (userRole === "cashier") {
      return ["/", "/sales"].includes(item.path);
    }
    if (userRole === "worker") {
      return ["/", "/sales", "/inventory", "/parties"].includes(item.path);
    }
    return false;
  });
